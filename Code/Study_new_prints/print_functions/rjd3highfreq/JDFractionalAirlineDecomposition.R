...
fractionalAirlineDecomposition_new <- function(y, period, sn = F, stde = F, nbcasts = 0, nfcasts = 0) 
{
...
jd2r_fractionalAirlineDecomposition_new <- function (jrslt, sn = F, stde = F, period) 
{
...
multiAirlineDecomposition_new <- function(y, periods, ndiff = 2, ar = F, stde = F, nbcasts = 0, 
                                           nfcasts = 0) 
{
    if (length(periods) == 1) {
    ...
    if (length(periods) == 1) {
        return(jd2r_fractionalAirlineDecomposition_new(jrslt, F, 
                                                   stde, periods))
    }
    else {
...
jd2r_multiAirlineDecomposition_new <- function (jrslt, stde = F, periods) 
{
    ncmps <- rjd3toolkit::.proc_int(jrslt, "ucarima.size")
    model <- rjd3highfreq:::arima_extract(jrslt, "ucarima_model")
    cmps <- lapply(1:ncmps, function(cmp) {
