...
for (col_name in names(nsa_df)[-1]) {
  ...
  if (sa_table$type[i] == "D") {
    ...
    time_series_list[[col_name]] <- ts(nsa_df[[col_name]][start_index:end_index], start=convert_date_string(start_date),
...
ts.plot(window(sa_loanshhcc,start = 2003, end=2014), window(nsa_loanshhcc,
                                    start = 2003,end=2014),
        window(rjd_odhh,start = 2003,end=2014), gpars = list(col = c("blue", "red", "black")))
