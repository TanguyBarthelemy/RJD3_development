new_name <- function(
    ...
    if (complexity <= 0) {
        stop("La complexité doit être positive.")
    }
    ...
    if (with_capital_letter) {
        reservoir <- c(reservoir, LETTERS)
    }
    if (with_lowercase_letter) {
        reservoir <- c(reservoir, letters)
    }
    if (with_numbers) {
        reservoir <- c(reservoir, 0:9)
    }

    return(paste0(
        sample(reservoir, size = complexity, replace = TRUE),
        collapse = ""
    ))
}
