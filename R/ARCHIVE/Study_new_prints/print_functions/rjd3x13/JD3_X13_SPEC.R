print_JD3_X13_SPEC <- function(
    ...
    if (x$benchmarking$enabled) {
        ...
        cat(
            ...
            ifelse(
                x$benchmarking$target == "TARGET_CALENDARADJUSTED",
                " (Auto)",
                ""
            ),
            "\n",
        ...
        cat(
            ...
            ifelse(x$benchmarking$lambda == 1, " (Auto)", ""),
            "\n",
            sep = ""
        )
        cat(
            "Rho: ",
            x$benchmarking$rho,
            ifelse(x$benchmarking$rho == 1, " (Auto)", ""),
            "\n",
            sep = ""
        )
        cat(
            "Bias: ",
            x$benchmarking$bias,
            ifelse(x$benchmarking$bias == "BIAS_NONE", " (Auto)", ""),
            "\n",
            sep = ""
        )
        cat(
            "Use forecast: ",
            ifelse(x$benchmarking$forecast, "Yes", "No (Auto)"),
            "\n",
            sep = ""
        )
    } else {
