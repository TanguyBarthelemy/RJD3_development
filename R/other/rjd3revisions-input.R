...
vintages <- rjd3revisions::create_vintages(df, 4L)

vertical_1 <- vintages$vertical_view
vertical_2 <- t(vintages$horizontal_view)
horizontal_1 <- vintages$horizontal_view
...
# - pas de java, tout en R
# - les dates sont uniquement en Date
# - pas de ts --> perte du caractère temporelle pour les séries vertical view et diagonal view
# - retrait de la possibilité de filtrer sur la période de révison. Je n'en vois aucune utilité

# Les discussion à avoir :

# - utilité de passer des choses en ts (vertical / diagonal) ?
# - utilité de passer des objets en data.frame (toutes les vues) ?
# - utilité de garder une colonne pour les rownames dans le cas d'un data.frame ?
# - utilité de garder le format 2020Q3 pour les tableaux (hors affichage) ?
