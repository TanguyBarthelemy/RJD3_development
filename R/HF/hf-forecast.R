...
pre_pro <- rjd3highfreq::fractionalAirlineEstimation(
    y = df$y,
    x = head(cal_reg, n = nrow(df)),
...
y_forecast_rjdverse <- amb.multi$decomposition$y |> tail(n = nb_forecast)
t_forecast_rjdverse <- amb.multi$decomposition$t |> tail(n = nb_forecast)
sa_forecast_rjdverse <- amb.multi$decomposition$sa |> tail(n = nb_forecast)
s7_forecast_rjdverse <- amb.multi$decomposition$s_7 |> tail(n = nb_forecast)
s365_forecast_rjdverse <- amb.multi$decomposition$s_365.2425 |> tail(n = nb_forecast)
i_forecast_rjdverse <- amb.multi$decomposition$i |> tail(n = nb_forecast)
...
future <- make_future_dataframe(m, periods = nb_forecast) |>
    tail(n = nb_forecast)
...
y_forecast_prophet <- forecast$yhat
s7_forecast_prophet <- forecast$weekly
s365_forecast_prophet <- forecast$yearly
t_forecast_prophet <- forecast$trend
sa_forecast_prophet <- y_forecast_prophet - s7_forecast_prophet - s365_forecast_prophet
i_forecast_prophet <- sa_forecast_prophet - t_forecast_prophet
c_forecast_prophet <- forecast$holidays
...
y_msts <- msts(df$y, seasonal.periods = c(7, 365.2425))
