...
# remotes::install_local("RJD3_development/HF_simulations/tssim_0.2.15.tar.gz")
...
tic()
source("RJD3_development/HF_simulations/UC_on_Sim_data_additive_det_S1.R")
gamma_1 <- gamma

source("RJD3_development/HF_simulations/UC_on_Sim_data_additive_det_S2.R")
gamma_2 <- gamma

source("RJD3_development/HF_simulations/UC_on_Sim_data_additive_det_S3.R")
gamma_3 <- gamma

source("RJD3_development/HF_simulations/UC_on_Sim_data_additive_det_S4.R")
gamma_4 <- gamma
sigma_4 <- sigma2

source("RJD3_development/HF_simulations/UC_on_Sim_data_additive_det_S5.R")
gamma_5 <- gamma
sigma_5 <- sigma2

source("RJD3_development/HF_simulations/UC_on_Sim_data_additive_det_S6.R")
gamma_6 <- gamma
sigma_6 <- sigma2
toc()
...
ft <- df |> as_grouped_data(groups = c("type")) |> 
  ...
  hline(i = c(nb_method + 1L, 2L * nb_method + 1L, 3L * nb_method + 1L, 4L * nb_method + 2L, 5L * nb_method + 2L)) |> 
  set_header_labels(
    gamma = "\u03B3",
    sigma = "\u03C3\u00B2",
    method = "Method",
  ...
  colformat_double(j = c("trend_RMSE", "weekly_RMSE", "yearly_RMSE", "remainder_RMSE", "sa_RMSE")) |> 
