...
add_sa_item(
    ...
    name = "series_4",
    x = y,
    rjd3tramoseats::tramoseats_spec("RSAFull")
)
rws <- read_workspace(jws)
...
x13_spec_d <- rjd3toolkit::set_transform(x13_spec, fun = "Log", outliers = TRUE)
# writing the specification in a xml file
...
# forcing multiplicative model
reg_arima_spec_d <- rjd3toolkit::set_transform(
    reg_arima_spec,
    fun = "Log",
    outliers = TRUE
)
