...
France
composite_calendar <- weighted_calendar(
    list(France, Belgium),
    weights = c(1, 2)
)
...
class(BE)
lt <- long_term_mean(BE, 12, groups = c(1, 1, 1, 1, 1, 0, 0), holiday = 7)
...
new_spec <- rjd3toolkit::add_outlier(
    init_spec,
    type = "AO",
    date = "2012-01-01"
)
...
init_spec <- rjd3x13::spec_x13("RSA5c")
new_spec <- set_automodel(init_spec, enabled = FALSE, acceptdefault = TRUE)
...
library("rjd3toolkit")
init_spec <- rjd3x13::spec_x13("RSA5c")
new_spec <- set_tradingdays(init_spec, option = "TD4", test = "None")
...
spec_x13_d <- rjd3x13::spec_x13("rsa5c")
spec_x13_d <- set_tradingdays(spec_x13_d, option = "TRADINGDAYS", test = "None")
