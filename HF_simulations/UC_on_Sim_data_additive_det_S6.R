...
# irregular
gamma <-0.6
si <- 0.6*150 # function needed input (param/150)

# shock sigma : 0 when deterministic
# on weekly

sigma2 <-0.075
sig2 <- 1.5 # function needed input (param/20)
...
for(j in 1:nb_series){
  x <- sim_daily_mstl(12, allComponents = TRUE,
...
x11.doy <- rjd3x11plus::x11plus(
  ...
  seas.s0 = "S3X1", seas.s1 = "S3X1", 
  extreme.lsig = 1.5, extreme.usig = 2.5)

end <- Sys.time()
list_time_X11[[j]] <- round(as.numeric(difftime(end, start, units = "secs")), 3)
 
#store s7
list_X11_s7[[j]]<- x11.dow$decomposition$s
# RMSE for X11 (vs True) S7 
list_RMSE_X11_s7[[j]]<-sqrt(mean((list_X11_s7[[j]]-list_true_s7[[j]])^2))

#store s365
list_X11_s365[[j]]<- x11.doy$decomposition$s
# RMSE for X11 (vs True) S365
list_RMSE_X11_s365[[j]]<-sqrt(mean((list_X11_s365[[j]]-list_true_s365[[j]])^2))
...
list_RMSE_X11_sa[[j]]<-sqrt(mean((list_X11_sa[[j]]-list_true_sa[[j]])^2))
...
for(j in 1:nb_series){
  start<-Sys.time()
  stl.dow <- rjd3stl::stlplus(y = list_raw[[j]], 
                              period = 7, 
                              multiplicative = FALSE, swindow = 3, twindow = 9, # 3= half, t= env p ?
                              ninnerloop = 2, nouterloop = 0, 
                              # nojump = FALSE, # not used any more?
                              weight.threshold = 0.001, weight.function = "BIWEIGHT")
  # Extract DOY pattern from DOW-adjusted data : run on SA 
  stl.doy <- rjd3stl::stlplus(y = stl.dow$decomposition[, "sa"], 
                              period = 365.2425, 
                              multiplicative = FALSE, swindow = 11, twindow = 169, 
                              ninnerloop = 2, nouterloop = 0,
                              # nojump = FALSE,
                              weight.threshold = 0.001, weight.function = "BIWEIGHT")
  end <- Sys.time()
  list_time_stl[[j]] <- round(as.numeric(difftime(end, start, units = "secs")), 3)
  
  #store s7
  list_stl_s7[[j]]<- stl.dow$decomposition[, "s"]
  # RMSE for STL (vs True) S7
  list_RMSE_stl_s7[[j]]<-sqrt(mean((list_stl_s7[[j]]-list_true_s7[[j]])^2))
    
  #store s365
  list_stl_s365[[j]]<- stl.doy$decomposition[, "s"]
  # RMSE for STL (vs True) S365
  list_RMSE_stl_s365[[j]]<-sqrt(mean((list_stl_s365[[j]]-list_true_s365[[j]])^2))
  ...
  list_RMSE_stl_sa[[j]]<-sqrt(mean((list_stl_sa[[j]]-list_true_sa[[j]])^2))
...
for(j in 1:nb_series){
  ...
  list_time_amb[[j]] <- round(as.numeric(difftime(end, start, units = "secs")), 3)


  #store s7
  list_amb_s7[[j]]<- amb.dow$decomposition$s
  # RMSE for amb (vs True) S7
  list_RMSE_amb_s7[[j]]<-sqrt(mean((list_amb_s7[[j]]-list_true_s7[[j]])^2))
  
  #store s365
  list_amb_s365[[j]]<- amb.doy$decomposition$s
  # RMSE for amb (vs True) S365
  list_RMSE_amb_s365[[j]]<-sqrt(mean((list_amb_s365[[j]]-list_true_s365[[j]])^2))
  ...
  list_RMSE_amb_sa[[j]]<-sqrt(mean((list_amb_sa[[j]]-list_true_sa[[j]])^2))
...
for(j in 1:nb_series){
  ...
  list_time_Mamb[[j]] <- round(as.numeric(difftime(end, start, units = "secs")), 3)
  ...
  # RMSE for amb (vs True) S7
  list_RMSE_Mamb_s7[[j]]<-sqrt(mean((list_Mamb_s7[[j]]-list_true_s7[[j]])^2))
  
  #store s365
  list_Mamb_s365[[j]]<- amb.multi$decomposition$s_365.2425
  # RMSE for amb (vs True) S365
  list_RMSE_Mamb_s365[[j]]<-sqrt(mean((list_Mamb_s365[[j]]-list_true_s365[[j]])^2))
  ...
  # RMSE for amb (vs True) SA
  list_RMSE_Mamb_sa[[j]]<-sqrt(mean((list_amb_sa[[j]]-list_true_sa[[j]])^2))
...
# Seasonal periodicities (in days)
  ...
  z_TBATS <- msts(list_raw[[j]], 
                  seasonal.periods = seas_per, 
                  start = decimal_date(as.Date(start_date)))
  #results
  start<-Sys.time()
  res <- tbats(z_TBATS, 
               use.box.cox = FALSE,
               use.damped.trend = FALSE) # desp options 
  end <- Sys.time()
  list_time_Tbats[[j]] <- round(as.numeric(difftime(end, start, units = "secs")), 3)
  ...
  # RMSE for amb (vs True) S7
  list_RMSE_Tbats_s7[[j]]<-sqrt(mean((list_Tbats_s7[[j]]-list_true_s7[[j]])^2))
  
  #store s365
  list_Tbats_s365[[j]]<- as.numeric(comps_tbats[,4])
  # RMSE for amb (vs True) S365
  list_RMSE_Tbats_s365[[j]]<-sqrt(mean((list_Tbats_s365[[j]]-list_true_s365[[j]])^2))
  
  #store t
  list_Tbats_t[[j]]<- as.numeric(comps_tbats[,2])
  # RMSE for amb (vs True) T
  list_RMSE_Tbats_t[[j]]<-sqrt(mean((list_Tbats_t[[j]]-list_true_t[[j]])^2))
  
  #store i
  list_Tbats_i[[j]]<- as.numeric(comps_tbats[,1])-as.numeric(comps_tbats[,2])-as.numeric(comps_tbats[,3])-as.numeric(comps_tbats[,4])
  # RMSE for amb (vs True) T
  list_RMSE_Tbats_i[[j]]<-sqrt(mean((list_Tbats_i[[j]]-list_true_i[[j]])^2))
  
  #store sa
  list_Tbats_sa[[j]]<- as.numeric(comps_tbats[,1])-as.numeric(comps_tbats[,3])-as.numeric(comps_tbats[,4])
  # RMSE for X11 (vs True) SA
  list_RMSE_Tbats_sa[[j]]<-sqrt(mean((list_Tbats_sa[[j]]-list_true_sa[[j]])^2))
...
for(j in 1:nb_series){
  history <- data.frame(ds = seq(as.Date(start_date), as.Date(end_date), 
                                 by = 'd'),
                        y = list_raw[[j]])
  colnames(history) <- c("ds","y")
  ### additive 
  start<-Sys.time()
  m1<- prophet(df = history,
               growth = "linear",
  ...
  list_time_Prophet[[j]] <- round(as.numeric(difftime(end, start, units = "secs")), 3)
  ...
  # RMSE for amb (vs True) S7
  list_RMSE_Prophet_s7[[j]]<-sqrt(mean((list_Prophet_s7[[j]]-list_true_s7[[j]])^2))
  
  #store s365
  list_Prophet_s365[[j]]<- forecast$yearly[1:nobs]
  # RMSE for amb (vs True) S365
  list_RMSE_Prophet_s365[[j]]<-sqrt(mean((list_Prophet_s365[[j]]-list_true_s365[[j]])^2))
  
  #store t
  list_Prophet_t[[j]]<- forecast$trend[1:nobs]
  # RMSE for amb (vs True) T
  list_RMSE_Prophet_t[[j]]<-sqrt(mean((list_Prophet_t[[j]]-list_true_t[[j]])^2))
  
  #store i
  list_Prophet_i[[j]]<- forecast$yhat[1:nobs]-forecast$trend[1:nobs]-forecast$yearly[1:nobs]-forecast$weekly[1:nobs]
  # RMSE for amb (vs True) I
  list_RMSE_Prophet_i[[j]]<-sqrt(mean((list_Prophet_i[[j]]-list_true_i[[j]])^2))
  
  #store sa
  list_Prophet_sa[[j]]<- forecast$yhat[1:nobs]-forecast$yearly[1:nobs]-forecast$weekly[1:nobs]
  # RMSE for X11 (vs True) SA
  list_RMSE_Prophet_sa[[j]]<-sqrt(mean((list_Prophet_sa[[j]]-list_true_sa[[j]])^2))
...
for(j in 1:nb_series){
  z_MSTL <- msts(list_raw[[j]], 
                 seasonal.periods = seas_per, 
                 start = decimal_date(as.Date(start_date)))
  start<-Sys.time()
  comps_MSTL<-mstl(z_MSTL)
  end <- Sys.time()
  list_time_MSTL[[j]] <- round(as.numeric(difftime(end, start, units = "secs")), 3)
  # str(comps_MSTL)

  #store s7
  list_MSTL_s7[[j]]<- as.numeric(comps_MSTL[,3])
  # RMSE for amb (vs True) S7
  list_RMSE_MSTL_s7[[j]]<-sqrt(mean((list_MSTL_s7[[j]]-list_true_s7[[j]])^2))
  
  #store s365
  list_MSTL_s365[[j]]<- as.numeric(comps_MSTL[,4])
  # RMSE for amb (vs True) S365
  list_RMSE_MSTL_s365[[j]]<-sqrt(mean((list_MSTL_s365[[j]]-list_true_s365[[j]])^2))
  ...
  #store i
  list_MSTL_i[[j]]<- as.numeric(comps_MSTL[,1])-as.numeric(comps_MSTL[,2])-as.numeric(comps_MSTL[,3])-as.numeric(comps_MSTL[,4])
  # RMSE for amb (vs True) I
  list_RMSE_MSTL_i[[j]]<-sqrt(mean((list_MSTL_i[[j]]-list_true_i[[j]])^2))
  
  #store sa
  list_MSTL_sa[[j]]<- as.numeric(comps_MSTL[,1])-as.numeric(comps_MSTL[,3])-as.numeric(comps_MSTL[,4])
  # RMSE for X11 (vs True) SA
  list_RMSE_MSTL_sa[[j]]<-sqrt(mean((list_MSTL_sa[[j]]-list_true_sa[[j]])^2))
...
# stockage scenario _s6
s7_s6<-c(avg_x11_s7,avg_stl_s7,avg_amb_s7, avg_Mamb_s7,avg_Tbats_s7,avg_Prophet_s7,avg_MSTL_s7)
s7_s6

s365_s6<-c(avg_x11_s365,avg_stl_s365,avg_amb_s365, avg_Mamb_s365,avg_Tbats_s365,avg_Prophet_s365,avg_MSTL_s365)
s365_s6

t_s6<-c(avg_x11_t,avg_stl_t,avg_amb_t, avg_Mamb_t,avg_Tbats_t,avg_Prophet_t,avg_MSTL_t)
t_s6

i_s6<-c(avg_x11_i,avg_stl_i,avg_amb_i, avg_Mamb_i,avg_Tbats_i,avg_Prophet_i,avg_MSTL_i)
i_s6

sa_s6<-c(avg_x11_sa,avg_stl_sa,avg_amb_sa, avg_Mamb_sa,avg_Tbats_sa,avg_Prophet_sa,avg_MSTL_sa)
sa_s6

time_s6<-c(avg_x11_time,avg_stl_time,avg_amb_time, avg_Mamb_time,avg_Tbats_time,avg_Prophet_time,avg_MSTL_time)
time_s6
