...
regs_cjo_N_1 <- data.frame(series = get_all_names(sap), selected_N_1 = character(count(sap))) # cree vecteur vide
...
for (k in seq_len(count(sap))) { #
    ...
    if (any(grepl("LeapYear", regressors))) regs_cjo <- paste0(regs_cjo, "_LY")
...
selected_N_outliers <- read.csv(file = "./selected_cjo_N_outliers.csv", sep = ";")
...
write.table(choix_regs_cjo, file = "./choix_cjo.csv", sep = ";", row.names = FALSE)
