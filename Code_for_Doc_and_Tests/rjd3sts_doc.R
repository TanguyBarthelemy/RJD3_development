...
seasDOY<- seasonal("seasDOY", period = 365.2424, type = "Trigonometric", 
                                  variance = .01, fixed = FALSE)
...
out.usr = matrix(round(cbind(pre.est[1:n.usr],
                             pre.esd[1:n.usr],
                             pre.tst[1:n.usr]), 
                       digits = c(rep(3, n.usr), rep(4, 2 * n.usr))), ncol = 3)
dimnames(out.usr) = list(colnames(births.reg), 
                         c("Est.", "SE", "t-stat."))
out.usr
...
out.otl = data.frame("Type"    = typ.otl,
...
round(c(births.bsm$bsm$final$level, births.bsm$bsm$final$slope, births.bsm$bsm$final$seasonal, 
        births.bsm$bsm$final$noise), 7)
